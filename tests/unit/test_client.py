...
@respx.mock
async def test_get_market_overview_top_n(sample_market_response):
    route = respx.get("https://api.coingecko.com/api/v3/coins/markets").mock(
        return_value=httpx.Response(200, json=sample_market_response)
    )
    async with CoinGeckoClient() as client:
        await client.get_market_overview(currency="usd", top_n=5)
...
@respx.mock
async def test_get_market_overview_rate_limit():
    respx.get("https://api.coingecko.com/api/v3/coins/markets").mock(
        return_value=httpx.Response(429, json={})
    )
    with pytest.raises(ValueError, match="Rate limit exceeded"):
        async with CoinGeckoClient() as client:
            await client.get_market_overview("usd", 10)